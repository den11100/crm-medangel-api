...
### POST `/api/medangel/appointment/create`

### Позволяет занять слот
...
| `med_analysis_ids`  | array   | Уникальные идентификаторы Услуг из Медангел |
| `med_appointment_id`| int     | Уникальные идентификаторы Записи из Медангел|
...
| `p_phone`           | string  | (только цифры  79001234567)                 |
...
{
    "status": "fail",
    "error": "Не переданы обязательные параметры запроса",
    "code": 10
}
```
...
### POST `/api/medangel/appointment/delete`
...
## Обновить данные по записи

### POST `/api/medangel/appointment/update`
